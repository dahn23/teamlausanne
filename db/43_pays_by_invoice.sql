-- 43_pays_by_invoice.sql
-- Coach « sur facture » (indépendant) : people.pays_by_invoice. Il reste dans Heures (ligne rose, montant attendu =
-- heures × tarif + extra, pour vérifier sa facture quand elle arrive) mais il est EXCLU du décompte PDF envoyé à la
-- fiduciaire et de « Valider les salaires » (il est payé via sa propre facture, reçue dans Factures).
alter table public.people add column if not exists pays_by_invoice boolean not null default false;
...
begin
  ...
  for r in select s.id, s.net, pe.first_name, pe.last_name, nullif(trim(pe.iban),'') as iban, pe.standing_order
           ...
           where s.ym = p_ym and s.invoice_id is null and s.net is not null and s.net > 0
             and not coalesce(pe.pays_by_invoice, false) loop
