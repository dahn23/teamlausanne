-- 39_salary_extra.sql
-- Heures : colonne « Extra (brut) » par personne et par mois (salary_slips.extra), ajoutée au montant
-- (salaire brut OU heures × tarif). staff_hours_month renvoie 'extra' pour coachs et profs.
alter table public.salary_slips add column if not exists extra numeric(12,2);
alter table public.salary_slips add column if not exists extra_note text;
