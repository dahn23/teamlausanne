-- 36_salary_monthly.sql
-- Coach SALARIÉ : people.salary_monthly (salaire brut mensuel, CHF). Null = payé à l'heure (coach_rates).
-- Dans l'onglet Heures, un salarié garde son décompte d'heures mais affiche « Salarié » + salaire brut
-- à la place de tarif/h + montant. staff_hours_month renvoie 'salary'. Mariano Palena = 7000.
alter table public.people add column if not exists salary_monthly numeric;
update public.people set salary_monthly = 7000 where first_name ilike 'Mariano' and last_name ilike 'Palena' and salary_monthly is null;
