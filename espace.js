...
function renderProfil() {
  ...
  for (const y of YOUTHS) {
    cards += `<div class="pt-prof-card">
      <div class="pt-youth-av big">${initials(y.first_name, y.last_name).toUpperCase()}</div>
      <div><b>${y.first_name} ${y.last_name}</b><div class="muted">Suivi mental & études — bientôt visible ici.</div></div>
    </div>`;
  }
  $("view-profil").innerHTML = `${cards || `<div class="pt-empty"><p>Aucun profil lié à ce compte.</p></div>`}
    <div class="pt-empty"><p>👤 Le détail des profils (mental & études publics) arrive à la prochaine étape.</p></div>`;
}
